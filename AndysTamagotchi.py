...
# Creating the game display and implementing time and system font
...
class Happiness(pygame.sprite.Sprite):
    ...
    def happinessLevel(self):   
        self.happiness_level = ""
        if self.current_happiness >= self.max_happiness:
            self.happiness_level = "High"
        elif self.current_happiness >= 750 and self.current_happiness <= 350:
            self.happiness_level = "Medium"
        elif self.current_happiness <= 350 and self.current_happiness >= 0:
            self.happiness_level = "Low"
    """
...
def main_loop():
    ...
    get_sad = USEREVENT + 0
    pygame.time.set_timer(get_sad, 3000)
    ...
    while game_execution:
        ...
        for event in pygame.event.get():
            ...
            if event.type == get_sad:
                happiness_bar.sprite.getSad(20)
                
                
        # Make cat move left and right  
        if cat_rect.left <= 100 or cat_rect.right >= 50:
            direction *= -1
            speed_x = random.randint(1, 2) * direction
        ...
        # Place icons on display screen
        screen.blit(cat, cat_rect)
        
        # Add happiness bar text
        Happiness().writeText()
        
        # Give buttons functionalty
        if food_btn.draw():
